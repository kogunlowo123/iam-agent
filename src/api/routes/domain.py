"""IAM Agent - Domain-Specific API Routes."""
...
@router.post("/api/v1/iam/analyze", summary="Run analysis")
async def analyze(request: Request):
    """Run analysis"""
    body = await request.json() if request.method in ("POST", "PUT", "PATCH") else {}
    logger.info("analyze_called", params=list(body.keys()) if body else [])
    # Domain-specific handler for IAM Agent
    return {
        "status": "success",
...
@router.post("/api/v1/iam/scan", summary="Scan target")
...
@router.post("/api/v1/iam/report", summary="Generate report")
...
@router.post("/api/v1/iam/remediate", summary="Execute remediation")
...
@router.get("/api/v1/iam/status", summary="Get status")
